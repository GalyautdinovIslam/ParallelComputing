// Написать программу, вычисляющую элементы вектора Z по формуле Zi = Xi * Yi. Векторы X, Y задаются на
// нулевом процессе и равными блоками пересылаются остальным процессам. Пересылка данных осуществляется функцией
// MPI_Send. Все процессы по формуле вычисляют свои элементы массива Z.
...
int main(int argc, char *argv[]) {
    ...
    if (rank == source) {
        ...
        for (int dest = 1; dest < size; dest++) {
            ...
            MPI_Recv(&z[(dest - 1) * count], amount, datatype, dest, tagX + dest, comm, ignore);
        }
